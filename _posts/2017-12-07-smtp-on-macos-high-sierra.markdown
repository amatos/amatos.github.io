...
So, for posterity, assuming you’re using [Gmail][1], and on MacOS High Sierra (nb: This still works on Catalina too):
...
- Regarding relayhost, enclosing smtp.gmail.com in square brackets prevents Postfix from doing an MX lookup on the hostname.  Google explicitly tells you to use [smtp.gmail.com in their docs][2], so no need to do an MX lookup on it.
...
Replace the username with yours, and use an [App Password][3].  Google will fail your authentication attempt if you try it with your normal Google password.
...
[2]:	https://support.google.com/a/answer/2956491?hl=en
